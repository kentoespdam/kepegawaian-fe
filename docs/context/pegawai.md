...
- ⏳ CRUD Data Pegawai (desain DIKUNCI, siap dikerjakan agen — `bd`: `9hk` Tambah, `ds5` Edit Profil/Gaji, `7h2` Toolbar filter). **Tambah** = halaman `/kepegawaian/data/tambah`, form adaptif by `statusPegawai` (NON_PEGAWAI sembunyikan seksi Kepegawaian + relax 3 FK — keputusan FE sengaja, BE smart-route via `isPegawai=false`), `POST /pegawai`. **Edit** = 2 Sheet (`PATCH /profil` 17 field + `PATCH /gaji` 9 field), submit `dirtyFields`-only. Validasi Zod `superRefine` by status. ⚠️ Mutation pegawai = **raw `fetch`** ke `/api/proxy/pegawai...` (client `api.*` master-only, tak ada `api.patch`). Hapus = out of scope (via terminasi). Lihat [CLAIM-ORDER-data-pegawai-crud.md](../CLAIM-ORDER-data-pegawai-crud.md).
