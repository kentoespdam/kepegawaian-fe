...
- **Tier 1 — icon rail** (narrow, far left): one icon per **module** (master, kepegawaian, cuti, laporan, penggajian, sistem). The active module is highlighted in **Evergreen (hijau pinus)**. The rail is **always visible** — it never collapses away, so navigation is constant and predictable (important for elderly users). *(If `riwayat` becomes a 7th module — open question in the context map above — this rail grows to 7.)*
...
**60:30:10:** content surface = 60% (white/near-white), the sidebar (rail + panel) = 30% (neutral), Evergreen (hijau pinus) = 10% accent (active module/entity, primary buttons, focus rings). Release 1 wires only the **master** module's entity panel; the other five rail icons are present but their panels are stubs (built in later grillings).
...
- **60% canvas** — off-white HANGAT (bukan biru-putih): `--background: oklch(0.99 0.008 95)`. Kurangi silau, cocok lensa menua yang tak mempersepsi putih-murni sebagai putih.
...
- **10% aksen** — Evergreen (hijau pinus) `--primary: oklch(0.48 0.09 158)` HANYA di area kecil (tombol primer, item aktif, focus ring); `--ring` sama, `--primary-dark: oklch(0.36 0.08 158)`. Aman jenuh justru karena langka. **Jangan** dipakai untuk teks tubuh kecil di atas putih.
...
- Nilai final ini (palet **Evergreen**, terkunci di `globals.css :root` + `.dark`) lolos contrast-gate; angka di atas adalah baseline yang boleh di-fine-tune saat implementasi ASAL lolos gate kontras di `### Accessibility & contrast`.
