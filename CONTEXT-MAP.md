...
- **RBAC:** entities the role can't `view` are not rendered as sub-items; a **module group with no view-able entity at all is not rendered** (no empty groups). Release 1 → the sidebar shows only the **Master** group, open; the other five modules have no entities yet so they don't appear (future-module info lives on the dashboard landing §13, not the sidebar). A module appears automatically once its entities are grilled & wired. *(If `riwayat` becomes a 7th module — open question in the context map above — it's just another group.)*
