...
/**
 * Generic hook for riwayat CRUD pages (kontrak, sk, sp, mutasi).
 ...
 */
export function useRiwayatTable<T extends { id?: string | number | null }>({
	pegawaiId,
	entityPath,
	entityLabel,
	queryKeyPrefix,
	...
	const nav = (updates: Record<string, string | undefined>) => {
		...
		router.replace(`/kepegawaian/data/${pegawaiId}/riwayat/${entityPath}?${p.toString()}`);
	};
	...
	const onReset = () => {
		router.replace(`/kepegawaian/data/${pegawaiId}/riwayat/${entityPath}`);
	};
