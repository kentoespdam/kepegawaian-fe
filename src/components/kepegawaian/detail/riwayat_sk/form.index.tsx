...
const RiwayatSkFormComponent = () => {
	const { defaultValues, open, setOpen } = useRiwayatSkStore(
		useShallow((state) => ({
		...
		<Dialog open={open} onOpenChange={setOpen}>
			<DialogContent className="p-0">
				<DialogHeader className="px-4 pt-2 space-y-0">
					<DialogTitle>Data Surat Keputusan</DialogTitle>
				</DialogHeader>
				<Separator />
				<Form {...form}>
					<form onSubmit={form.handleSubmit(onSubmit)}>
						<div className="grid gap-4 max-h-[450px] overflow-auto pl-4 pr-2 pb-4">
							<InputZod
								type="number"
								id="id"
								label="ID"
