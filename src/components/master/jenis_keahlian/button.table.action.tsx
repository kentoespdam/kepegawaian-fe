...
type JenisKeahlianTableActionProps = {
	jenisKeahlianId: number;
};
const JenisKeahlianTableAction = ({
	jenisKeahlianId,
}: JenisKeahlianTableActionProps) => {
	const { setSelectedDataId, setOpenDelete } = useJenisKeahlianStore(
		useShallow((state) => ({
	...
	const editHandler = () => {
		router.push(`/master/jenis_keahlian/edit/${jenisKeahlianId}`);
	};

	const deleteHandler = () => {
		setSelectedDataId(jenisKeahlianId);
		setOpenDelete(true);
	};
...
export default JenisKeahlianTableAction;
