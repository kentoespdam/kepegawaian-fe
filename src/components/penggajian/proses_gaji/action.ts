...
export const verifikasiProses = async (formData: VerifikasiSchema) => {
	...
	switch (formData.phase) {
		...
		case "WAIT_VERIFICATION_PHASE_2":
			verifPath = "verify2"
			break
		case "WAIT_VERIFICATION_PHASE_3":
			verifPath = "accept"
			break
	}
	const apiUrl = `${API_URL}/penggajian/batch/${formData.id}/${verifPath}`
	const headers = setAuthorizeHeader(cookies())
...
export const reprocessGaji = async (formData: VerifikasiSchema) => {
	const apiUrl = `${API_URL}/penggajian/batch/${formData.id}/reprocess`
	const headers = setAuthorizeHeader(cookies())
	const response = await fetch(apiUrl, {
		method: "PATCH",
		headers,
