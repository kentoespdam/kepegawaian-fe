import type { KomponenGaji, KomponenGajiMini, KomponenGajiSchema } from "@_types/penggajian/komponen";
...
const AvailableCodeButton = ({ availableCode, form, currentCode }: AvailableCodeButtonProps) => {
        ...
        <div className="grid gap-2">
            ...
            <div className="flex gap-2">
                <TooltipBuilder text="tambah">
                    <Button size={"sm"} type="button" onClick={() => operatorClick("+")}>+</Button>
                </TooltipBuilder>
                ...
                {availableCode.map(item => item.kode === currentCode ? null : (
                    <div key={item.kode}>
                        <TooltipBuilder text={item.nama}>
                            <Button size={"sm"} onClick={() => kodeClick(item)} type="button">{item.kode}</Button>
                        </TooltipBuilder>
                    </div>)
                )}
            </div>
