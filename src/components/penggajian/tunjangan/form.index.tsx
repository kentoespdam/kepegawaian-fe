...
const TunjanganFormComponent = ({
	jenisTunjangan,
	data,
}: TunjanganFormComponentProps) => {
	const router = useRouter();
	const params = useSearchParams();
	const redirectTo = params.get("callback")
		? atob(params.get("callback") as string)
		: "";
	const search = new URLSearchParams(redirectTo);
	search.delete("callback");
	const { defaultValues, setDefaultValues } = useTunjanganStore(
		useShallow((state) => ({
	...
	useEffect(
		() => setDefaultValues(jenisTunjangan, data),
		[setDefaultValues, jenisTunjangan, data],
	);
