...
export function SettingClient() {
	...
	const handleSelectPegawai = (pegawaiId: number) => {
		...
		router.replace(`/penggajian/batch/${params.id}/setup?${p.toString()}`);
	};
	...
	return (
		<div className="flex flex-col lg:flex-row gap-4">
			...
			<div className="w-full lg:w-96 shrink-0 rounded-lg border bg-card shadow-sm">
				...
				<div className="max-h-150 overflow-y-auto">
					{isPending ? (
						...
						<div className="p-4 text-center text-sm text-muted-foreground">Belum ada data pegawai</div>
					) : (
								...
								{rows.map((row) => (
									...
									>
										...
										<div className="text-xs text-muted-foreground">
											{row.nipam} • {row.namaJabatan ?? "-"} • {row.golongan ?? "-"}
										</div>
									</button>
								))}
							</div>
			...
			{/* Panel kanan: Rincian Komponen */}
			<div className="flex-1 min-w-0">
				{selectedPegawaiId ? (
					<PegawaiKomponenPanel batchId={params.id} pegawaiId={selectedPegawaiId} />
				) : (
			...
			<div className="grid grid-cols-2 gap-4 text-sm">
				<div className="space-y-2">
					...
					<div className="flex justify-between">
						...
						<span className="font-medium">{formatRupiah(komponen.gajiPokok)}</span>
					</div>
					<div className="flex justify-between">
						<span>PHDP</span>
						<span className="font-medium">{formatRupiah(komponen.phdp)}</span>
					</div>
					<div className="flex justify-between border-t pt-2">
						<span className="font-semibold">Total Penghasilan Kotor</span>
						<span className="font-semibold">{formatRupiah(komponen.penghasilanKotor)}</span>
					</div>
				</div>
				...
				<div className="space-y-2">
					...
					<div className="flex justify-between">
						...
						<span className="font-medium">{formatRupiah(komponen.totalPotongan)}</span>
					</div>
					<div className="flex justify-between">
						<span>Pajak</span>
						<span className="font-medium">{formatRupiah(komponen.pajak)}</span>
					</div>
					<div className="flex justify-between border-t pt-2">
						<span className="font-semibold">Penghasilan Bersih</span>
						<span className="font-semibold">{formatRupiah(komponen.penghasilanBersihFinal)}</span>
					</div>
				</div>
			</div>
		</div>
