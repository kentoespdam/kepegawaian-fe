...
import { type JabatanMini, JabatanMiniSchema } from "@_types/master/jabatan";
import type { Organisasi, OrganisasiMini } from "@_types/master/organisasi";
...
export interface RiwayatMutasi {
	...
	jabatan: JabatanMini | null;
	profesi: ProfesiMini | null;
	golonganLama: Golongan | null;
	organisasiLama: OrganisasiMini | null;
	namaOrganisasiLama: string;
	jabatanLama: JabatanMini | null;
	namaJabatanLama: string;
	profesiLama: ProfesiMini | null;
...
export const RiwayatMutasiSchema = BaseRiwayatSkSchema.extend({
	...
	organisasiId: z.optional(z.number()),
	jabatan: JabatanMiniSchema.optional(),
	jabatanId: z.optional(z.number()),
	organisasiLamaId: z.number(),
	namaOrganisasiLama: z.string(),
	jabatanLamaId: z.number(),
	namaJabatanLama: z.string(),
	profesiId: z.optional(z.number()),
	profesiLamaId: z.number(),
	namaProfesiLama: z.string(),
	golonganLamaId: z.optional(z.number()),
	namaGolonganLama: z.string(),
	notes: z.string(),
}).superRefine((val, ctx) => {
	const { tanggalSk, tmtBerlaku, nipam, golonganLamaId } = val;
	if (!nipam.startsWith("KO-") && golonganLamaId && golonganLamaId <= 0) {
		ctx.addIssue({
