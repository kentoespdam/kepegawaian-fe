...
export const cookieStringToObject = (
	...
	return Object.assign({}, ...cookies) as RequestCookie;
};

export const setAuthCookieHeader = async (cookieString: string[]) => {
	(await cookies()).set("asdf-session", cookieString[0].split("=")[1], {});
};
...
export const getExpToken = (token: string): number => {
	...
	try {
		...
		if (e instanceof Error) console.error("getExpToken", e.message);
		return 0;
	}
};
...
export const extracNipamFromToken = async (): Promise<string | null> => {
...
export const setAuthorizeHeader = (
	...
	const authorizationToken =
		token ?? sessionCookies.get(sessionNames[2])?.value;
	return {
...
export const getNipamFromCookie = async () => {
	const cookieList = await cookies();
	const tokenString =
		cookieList.get(sessionNames[0])?.value ||
		cookieList.get(sessionNames[1])?.value;
