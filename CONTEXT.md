# CONTEXT — Kepegawaian FE (Perumdam Tirta Satria)

Ubiquitous language and resolved decisions for the Kepegawaian front-end.
Scope of this document (grilling round 1): **Authentication + Master module** only.

## Glossary

- **Kepegawaian** — the HR/staffing domain of Perumdam Tirta Satria (a regional water utility, PDAM). The app is its front office.
- **Master module** — reference/master-data management: the ~22 lookup entities the rest of the system references (golongan, jabatan, organisasi, grade, profesi, sanksi, etc.).
...
## Entity taxonomy (Master)

- **CRUD entities (~17)** — full create/read/update/delete: golongan, grade, jabatan, organisasi, profesi, sanksi, level, jenjang-pendidikan, jenis-keahlian, jenis-kitas, jenis-pelatihan, jenis-sp, alasan-berhenti, alat-kerja, apd, hari-libur, rumah-dinas.
- **Reference lists (5, read-only)** — jenis-kontrak, jenis-mutasi, jenis-sk, status-kerja, status-pegawai. **No Master pages** in release 1: they are consumed only as `/list` dropdown sources by other modules (kepegawaian, etc.) later. The Master module builds pages for the **17 CRUD entities only**. (If auditing their contents in-UI is later needed, a read-only page can be added — deferred.)
- **Tree entities** — `organisasi` and `jabatan` are self-referencing (`parentId`).
- **FK-dependent entities** — profesi→(organisasi,jabatan,grade); grade→level; apd/alat-kerja→profesi; sanksi→jenis-sp. These need `/list` endpoints to populate dropdowns.
...
- `GET /master/{entity}` — paginated + filtered: `page`, `size` (1..100), `sortBy`, `sortDirection` (asc|desc), plus per-field query filters.
- `GET /master/{entity}/list` — unpaginated, for dropdowns.
...
- **Memory guardrails (mandatory, from TanStack Query v5 docs):**
  - `gcTime` = default **5 min**: when a table's component unmounts (leave the page), its cache is **garbage-collected** — nothing accumulates across the 22 entities. At any instant only *one table + the comboboxes currently open* are live.
...
- **Rationale vs alternatives:** RSC-only means a full navigation (skeleton flash) per pagination and no client `/list` cache; the hybrid prefetch adds `dehydrate`/`HydrationBoundary` boilerplate ×22 entities — more code, more bug surface, against "simple, minimal bugs". Client Query gives smooth pagination + shared `/list` cache + auto-eviction with the least machinery.
...
### List-screen anatomy — toolbar tipis + tabel medium-density + sticky header (17 entitas Master)
Semua 17 halaman daftar Master memakai satu anatomi (opsi A yang disetujui), dirender lewat `<DataTable>` bersama:
...
- **UI enforcement rule (locked) — unmount, don't disable.** Unauthorized actions are **never rendered** (component returns `null`), not shown greyed-out or via `display:none`. Rationale: (1) cleanest for elderly — UI shows only what the user can actually do, zero disabled-grey clutter (disabled buttons are low-contrast, against `### Accessibility & contrast`); (2) lighter — the element, its handlers, and its state never enter the React tree, so idle cost is zero; (3) can't be "un-hidden" via inspect-element like a CSS-hidden node.
  ...
  - Applied consistently across all 17 entities via one small render helper `<Can action entity>…</Can>` that renders `null` when the check fails.
...
- **Tier 1 — icon rail** (narrow, far left): one icon per **module** (master, kepegawaian, cuti, laporan, penggajian, sistem). The active module is highlighted in **Tirta Blue**. The rail is **always visible** — it never collapses away, so navigation is constant and predictable (important for elderly users).
...
- **Breadcrumb** `Master / {Entitas}` (mis. "Master / Sanksi") sekaligus jadi judul halaman — halaman TIDAK mengulang `<h1>` besar. Di mobile menciut ke nama entitas aktif + hamburger `[≡]`.
...
### Form engine — React Hook Form + Zod via shadcn `<Field />` (satu `<CrudForm>` untuk 17 entitas)
Semua form CRUD Master memakai **React Hook Form (RHF) v7 + Zod** (`zodResolver`), dirender lewat komponen **shadcn `<Field />`** (di v4 `<Field />` sudah dipisah dari state library, jadi kompatibel dengan Base UI apa pun engine-nya). Dipilih di atas TanStack Form / native Server Actions karena paling stabil, paling ringan (uncontrolled/ref-based, ~9kB, re-render minimal), dan paling mudah untuk junior dev (pola `useForm({ resolver: zodResolver(schema) })` + `handleSubmit` ada di setiap contoh shadcn). Rincian trade-off ada di **ADR 0002**.
...
- **KISS:** field sanksi (8 switch) & profesi (FK) hanyalah deskriptor field, bukan form khusus. Junior cukup menulis skema Zod, tidak menyentuh mekanik form.
...
### Heavy-form layout — labeled sections + switch list for the flag-dense entities
The two form-heavy entities render inside a **Sheet** (right, ~480px) organized into **labeled sections**, never a flat field dump:
- **`sanksi`** (heaviest — 3 core fields + 8 boolean consequence flags + 1 conditional int):
  - **Section "IDENTITAS"** → `kode`, `keterangan`, `jenisSpId` (combobox).
  - **Section "KONSEKUENSI SANKSI"** → the 8 booleans as a **switch list** (Base UI Switch, one policy effect per row), each with a **plain-language Indonesian label** — NOT the raw field name: `potTkk`→"Potong TKK", `isPendingPangkat`→"Tunda kenaikan pangkat", `isPendingGaji`→"Tunda kenaikan gaji berkala", `isTurunPangkat`→"Turunkan pangkat", `isTurunJabatan`→"Turunkan jabatan", `isSuspension`→"Skorsing (suspension)", `isTerminateDh`→"PHK dengan hormat", `isTerminateTh`→"PHK tidak dengan hormat".
  - **`jmlPotTkk` is conditionally revealed** — hidden until `potTkk` is switched on (no dead disabled field sitting in the layout); appears indented under the "Potong TKK" row when active.
  - Required (backend): `kode`, `keterangan`, `jenisSpId`. All flags default `false`.
- **`profesi`** reuses the **same two-section grammar**: "IDENTITAS" (`nama`, plus `organisasiId`/`jabatanId`/`gradeId` comboboxes — all optional FKs) then "DETAIL" (the two long text areas `detail`, `resiko` as multi-line `<textarea>`). Required (backend): `nama`, `detail`, `resiko`.
- **Rule:** switches (not checkboxes) for independent on/off policy effects; every flag gets a human label; both heavy forms share one visual grammar so the pattern is learn-once. Simple entities keep the default single-column Dialog (per CRUD form presentation) — sectioning is only for these two.
...
### Dashboard landing — welcome page with Master shortcuts, extensible for future modules
Login lands on a **welcome page** (route `/` or `/dashboard`), **not** straight into a table:
- **Greeting header:** "Selamat datang, {nama}" + a one-line context ("Modul Master · data referensi"). Name comes from the DAL `account.get()` already in hand — no extra fetch.
- **Shortcut grid:** cards linking to the 17 Master CRUD entities (click → that entity's table). Static cards, **no per-entity count queries** (honest for release 1 — no invented pegawai statistics = no AI slop; and lightweight — the landing does ~0 data fetching).
